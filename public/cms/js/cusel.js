...
jQuery(document).ready(function(){
	...
	*/
	...
	var cuselFrame = '<div class="cusel '+chElClass+' '+classDisCusel+'"'+
					...
					' style=""'+
					' tabindex="'+chElTab+'"'+
					'><div class="cusel-repeat">'+
					'<div class="cuselFrameRight '+classDisCuselText+'"></div>'+
					'<div class="cuselText '+classDisCuselText+'">'+defaultText+'</div>'+
...
jQuery("html").click(
	function(e)
	{
		...
		if((clickedClass.indexOf("cuselText")!=-1 || clickedClass.indexOf("cuselFrameRight")!=-1) && clickedClass.indexOf("classDisCuselLabel")==-1)
		{
...
jQuery(".cusel").keypress(
function(event)
{
	/*
		функция отбора по нажатому символу
		keypress нужен для отлова символа нажатой клавишт
	*/
	var charKey;
		if(window.event) charKey=window.event.keyCode;
		else if (event) charKey=event.which;
		
		charKey = String.fromCharCode(charKey).toUpperCase();
		
	
			
		var arrOption = jQuery(this).find("span"),
		...
		{
			firstLetter = arrOption.eq(i).text().charAt(0).toUpperCase();
				if(firstLetter==charKey)
			{
				
				jQuery(this)
					.find(".cuselOptHover").removeClass("cuselOptHover")
					.end().find("span").eq(i).addClass("cuselOptHover")
					.end().end().find(".cuselText").eq(0).text(arrOption.eq(i).text());
