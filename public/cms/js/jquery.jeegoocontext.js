...
(function($){
    ...
    var _clearActive = function(){
        for(cm in _menus)
        {
			...
			$('.header').removeClass('active_contextmenu');
			$('.header span').removeClass('active_gradient');
        } 
    };
	...
	$.fn.jeegoocontext = function(id, options){
        ...
        return this[_menus[id].livequery ? 'livequery' : 'bind'](eventType, function(e){
            ...
            $(_menus[id].context).parents('div').eq(0).addClass('active_contextmenu');
			$(_menus[id].context).children('span').addClass('active_gradient');                       
            // Clear all hover state.
