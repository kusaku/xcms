...
(function($) {
	...
	$.widget("ui.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		_mouseDrag: function(event) {
			...
			itemBefore = this.placeholder[0].previousSibling; // Куда мы пытаемся опустить
			...
			} else if (itemBefore != null && itemBefore.nodeName == 'LI' && this.positionAbs.left > $(itemBefore).offset().left + this.options.tabSize) { // Работает когда нужно добавить ul!!!
				if (!($(itemBefore).is(this.options.disableNesting))) { // Если у элемента который принимает нет класса прописанного в опции disableNesting
					...
					$(this.placeholder[0]).addClass(this.options.errorClass).css('marginLeft', this.options.tabSize); // Нельзя опустить элемент в данную ul
				}
