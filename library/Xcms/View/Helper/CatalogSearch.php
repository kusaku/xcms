...
class Zend_View_Helper_CatalogSearch  extends Zend_View_Helper_Abstract {
    ...
    public function catalogSearch() {
        ...
        $etype_elem = $mcet->getModuleElementType('catalog', 'item');
        if( isset($etype_elem) ) {
            ...
            foreach($fgroups as $group) {
                foreach($group->getFields() as $field) {
                    if( ($field->name == 'name' || preg_match('/^catalog/', $field->name)  )&& !preg_match('/photo|price/', $field->name) )
                    $fields[$field->name] = $field->title;
                }
            }
            // Получаем категории
            $etype = $mcet->getModuleElementType('catalog', 'category');
            ...
            if (count($this->table) > 0 ) {
                ...
                // Получаем Url
                $etype = $mcet->getModuleElementType('catalog', 'search');
